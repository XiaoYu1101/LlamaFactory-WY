...
若启动长时间停留在初始化阶段，先执行 `python -u scripts/check_intent_env.py`。它打印当前 Python 路径和 Web 依赖版本，检查 Pydantic/Gradio 初始化，不监听端口、不加载模型。超过 30 秒会定时打印线程堆栈，可保留输出用于定位。Ctrl+C 导致的 KeyboardInterrupt 本身不是依赖不兼容的证据；不要仅凭该堆栈重装或降级整个环境。完整启动以 [3/3] 工作台路由已就绪为完成标志。
